...
volatile float samplesA [BUFSIZE][4] ;
...
void ADC_Handler (void) {
  ...
  if (ADC->ADC_ISR & ADC_ISR_EOC0) {  // ensure there was an End-of-Conversion and we read the ISR reg
    ...
    float temp = gain * (((float)(val - 2048) / 4096.0) - OFFSET_CHANNEL_A);    // stick in circular buffer
    samplesA [sptrA][0] = temp;//1*FC[0]*temp + 1*FC[1]*pastSamplesA[0] + 1*FC[2]*pastSamplesA[1]
  ...
  if (ADC->ADC_ISR & ADC_ISR_EOC2) {  // ensure there was an End-of-Conversion and we read the ISR reg
    ...
    samplesA[sptrB][1] = gain * (((float)(val - 2048) / 4096.0) - OFFSET_CHANNEL_B) ;   // stick in circular buffer
  ...
  if (ADC->ADC_ISR & ADC_ISR_EOC4) {  // ensure there was an End-of-Conversion and we read the ISR reg
    ...
    samplesA[sptrC][2] = gain * (((float)(val - 2048) / 4096.0) - OFFSET_CHANNEL_C) ;   // stick in circular buffer
  ...
  if (ADC->ADC_ISR & ADC_ISR_EOC6) {  // ensure there was an End-of-Conversion and we read the ISR reg
    ...
    samplesA[sptrD][3] = gain * (((float)(val - 2048) / 4096.0) - OFFSET_CHANNEL_D) ;   // stick in circular buffer
      ...
      for (int i = 0; i < 4; i++) {
        for (int k = 4; k < 70; k++ ) {
          for ( int n = 0; n < BUFSIZE; n++) {
            twiddleC = cos(2 * PI * k * n / 2047.0);
            twiddleS = sin(2 * PI * k * n / 2047.0);
            dft[k][0] += samplesA[n][i] * twiddleC;
            dft[k][1] += -1 * samplesA[n][i] * twiddleS;
          }
          phase[k][i] = (float)atan2(dft[k][1], dft[k][0]);
         // mag[k][i] = sqrt(powf(dft[k][0], 2) + powf(dft[k][1], 2));
        }//calc DFT
          Serial.print("Channel");
      }

      for (int i = 0; i < 70; i = i + 1) {
        sprintf(output, "%.4f,%.4f,%.4f,%.4f\n", phase[i][0], phase[i][1], phase[i][2], phase[i][3]);
        Serial.print(output);
      }
    }
    sptrA = sptrB = sptrC = sptrD = 0;
